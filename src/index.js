...
const payor = eosConfig.payorAccount
const payorPerms = eosConfig.payorPerms

async function init() {
  try {
    for (const account of watchAccounts) {

      console.log("Checking Existing Balance for Payor Account:", payor)
      const existingBalance = (await eosUtil.getCurrencyBalance("eosio.token", payor, "EOS"))
      existingBalanceNum = Number(existingBalance[0].split(' ')[0])
          ...
          [
            {
              account: 'eosio',
              name: 'powerup',
              authorization: [{ actor: payor, permission: payorPerms }],
              data: {
                payer: payor,
                receiver: account,
                days: 1,
      ...
      console.log('CPU available after Power-Up:', Number(cpuAfter).toLocaleString(), '\n')

      const netAfter = (await eosUtil.getAccount(account)).net_limit.available
      console.log('NET available after Power-Up:', Number(netAfter).toLocaleString(), '\n')
    }
  } catch (error) {
    console.error(error)
  }
